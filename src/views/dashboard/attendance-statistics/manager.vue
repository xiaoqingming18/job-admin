        ...
        <div class="filter-form">
          <el-form :inline="true" :model="queryParams" class="filter-form-content">
            <el-form-item label="项目">
              <el-select v-model="queryParams.projectId" placeholder="请选择项目" @change="handleProjectChange">
                <el-option
...
import { getProjectList, getManagerProjectList, getCompanyProjectList } from '@/api/project'
...
const loadProjects = async () => {
  try {
    ...
    if (userStore.isProjectManager) {
      // 项目经理只能查看自己管理的项目
      res = await getManagerProjectList(userId)
      projects.value = res.data || []
    } else if (userStore.isCompanyAdmin) {
      // 企业管理员查看企业的所有项目
      const companyId = userStore.userInfo?.companyId
      if (!companyId) {
        throw new Error('无法获取企业信息，请重新登录')
      }
      res = await getCompanyProjectList(companyId)
      projects.value = res.data || []
