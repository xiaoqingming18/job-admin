<template>
  <div class="leave-company-container">
    <!-- 页面标题 -->
    ...
    >
      <div v-if="currentLeave" class="approval-dialog-content">
        <div class="leave-info">
          ...
          <p v-if="currentLeave.pmApproverName"><strong>项目经理审批：</strong>{{ currentLeave.pmApproverName }} {{ currentLeave.pmApprovalStatusDesc }} ({{ currentLeave.pmApprovalTime }})</p>
          <p v-if="currentLeave.pmApprovalComments"><strong>项目经理意见：</strong>{{ currentLeave.pmApprovalComments }}</p>
        </div>
...
import { getCompanyLeaves, approveLeaveByAdmin, getPendingApprovalLeaves } from '@/api/leave'
...
const loadProjects = async () => {
  try {
    ...
    console.log('Company ID from userStore:', companyId)
    
    if (!companyId) {
      ElMessage.error('企业信息未找到')
      return
    }
    
    const res = await getCompanyProjectList(companyId)
    console.log('项目列表获取成功:', res.data)
...
const fetchLeaveList = async () => {
  ...
  try {
    ...
    if (activeTab.value === 'pending') {
      ...
      const companyId = userStore.userInfo?.companyId
      if (companyId) {
        const res = await getCompanyLeaves(companyId, params)
        leaveList.value = res.data.records || []
        total.value = res.data.total || 0
      } else {
        leaveList.value = []
