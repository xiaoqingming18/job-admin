...
const statusFilter = ref<'pending' | 'unsign' | 'review' | 'active' | 'expired' | ''>('')
  ...
  try {
    ...
    if (response.code === 0) {
      contractList.value = response.data.filter(contract => 
        contract.status === 'pending' || contract.status === 'unsign' || 
...
const handleSubmitSigning = async (contract: LaborContract) => {
  if (contract.status !== 'pending' && contract.status !== 'unsign') {
    ElMessage.warning('只有待发起签署或待签署状态的合同可以提交签订')
    return
  }
